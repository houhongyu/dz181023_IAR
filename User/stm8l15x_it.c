...
{
  //if(GPIO_ReadInputDataBit(GPIOD,GPIO_Pin_7)==SET)
   //{
    // Delay_Timer(100);
...
/**
  ...
  * @retval None
  */
INTERRUPT_HANDLER(TIM3_UPD_OVF_TRG_BRK_USART3_TX_IRQHandler,21)   // 延时中断
{
   //Delay_Timer_Decrement();
  ...
  if(TIM3_GetITStatus(TIM3_IT_CC1) == SET)
    {
        ...
        GPIO_ResetBits(GPIOD, GPIO_Pin_2);  //设为低电平
    }    
    else if(TIM3_GetITStatus(TIM3_IT_CC2) == SET)
      {
         TIM3_ClearITPendingBit(TIM3_IT_CC2);
         CapVal = TIM3_GetCapture2();
         temp=CapVal;
         GPIO_SetBits(GPIOD, GPIO_Pin_2);  //设为高电平
         TIM3_SetCounter(0);
         //Delay_Timer(10);      //1：延时CapVal后置PD2为低电平,此延时可能会影响PWM波形
